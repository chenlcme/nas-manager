...
export function ArtistsView({ onPlaySong, onShowSongDetail, onBatchEdit, playingSongId }: ArtistsViewProps) {
  ...
  async function toggleArtist(artistId: number, artistName: string) {
    ...
    try {
      const url = `/api/artists/${artistId}/songs?${SORT_BY_PARAM}=${encodeURIComponent(songSortBy)}&${ORDER_PARAM}=${encodeURIComponent(songOrder)}`;
  ...
  function handleSongSortChange(newSortBy: SortField, newOrder: SortOrder) {
    ...
    if (expandedArtist !== null) {
      ...
      debounceTimerRef.current = setTimeout(() => {
        ...
        const url = `/api/artists/${expandedArtist}/songs?${SORT_BY_PARAM}=${encodeURIComponent(newSortBy)}&${ORDER_PARAM}=${encodeURIComponent(newOrder)}`;
